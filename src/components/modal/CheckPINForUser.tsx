...
import { CheckPINModalProps } from './ModalProps';
...
export const CheckPINForUser: React.FC<CheckPINModalProps> = ({ visible, onCancel, onCreate, isLoading }) => {
    ...
    <Modal
      ...
      title={
        <div style={{ fontSize: isDesktop || isTablet ? FONT_SIZE.xl : FONT_SIZE.lg }}>
          {t('requests.checkPINForUserModalTitle')}
        </div>
      }
      onCancel={onCancel}
      maskClosable={true}
        ...
        <BaseForm.Item style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0' }}>
          <Space>
            ...
            <Button type="primary" style={{ height: 'auto' }} loading={isLoading} key="add" onClick={onOk}>
              <P1>{t('requests.checkPINForUserModal')}</P1>
            </Button>
          </Space>
        </BaseForm.Item>
      }
    >
      <BaseForm form={form} onFinish={onFinish} name="CheckPINForUserForm">
        <BaseForm.Item
          name={'pin'}

...
export const AddSubService: React.FC<CreateServiceModalProps> = ({ visible, onCancel, onCreateService, isLoading }) => {
    ...
    <Modal
      ...
      title={
        <div style={{ fontSize: isDesktop || isTablet ? FONT_SIZE.xl : FONT_SIZE.lg }}>
          {t('subServices.addSubServiceModalTitle')}
        </div>
      }
      onCancel={onCancel}
      maskClosable={true}
        ...
        <BaseForm.Item style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0' }}>
          <Space>
            ...
            <Button type="primary" style={{ height: 'auto' }} loading={isLoading} key="add" onClick={onOk}>
              <P1>{t('subServices.addSubServiceModalTitle')}</P1>
            </Button>
          </Space>
        </BaseForm.Item>
      }
    >
      <BaseForm form={form} onFinish={onFinish} name="SubServicesForm">
        <BaseForm.Item
